mutable struct TransformedMCMCEnsembleIterator{
    ...
    stepno::Int
    n_accepted::Int
    info::TransformedMCMCEnsembleIteratorInfo
    context::CTX
end
...
nsamples(chain::TransformedMCMCEnsembleIterator) = size(chain.samples, 1)

current_sample(chain::TransformedMCMCEnsembleIterator) = last(chain.samples)

sample_type(chain::TransformedMCMCEnsembleIterator) = eltype(chain.samples)

samples_available(chain::TransformedMCMCEnsembleIterator) = size(chain.samples,1) > 0

isvalidchain(chain::TransformedMCMCEnsembleIterator) = current_sample(chain).logd > -Inf
...
function TransformedMCMCEnsembleIterator(
    ...
    iter = TransformedMCMCEnsembleIterator(
        ...
        stepno,
        n_accepted,
        TransformedMCMCEnsembleIteratorInfo(id, cycle, false, false),
        context
    )
...
function transformed_mcmc_step!!(
    iters::Vector{},
    tuners::Vector{TransformedMCMCTransformTuner},
...
function transformed_mcmc_iterate!(
    ...
    transformed_mcmc_step!!(chains, tuners)
    return nothing
end
#=
# Unused?
