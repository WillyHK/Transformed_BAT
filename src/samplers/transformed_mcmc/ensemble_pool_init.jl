...
@with_kw struct TransformedMCMCEnsemblePoolInit <: TransformedMCMCInitAlgorithm
    init_tries_per_walker::ClosedInterval{Int64} = ClosedInterval(1, 128)
...
function mcmc_init!(
    ...
    new_ensembles = _gen_ensembles(rngpart, ncandidates .+ (one(Int64):nensembles), algorithm, density, initval_alg,nwalker,context)#*n,context)

    #new_tuners = get_tuner.(Ref(TransformedMCMCNoOpTuning()),new_ensembles) # NoOpTuner for BurnIn # TODO change this to FlowTuner
    new_temperers = fill(get_temperer(algorithm.tempering, density), size(new_ensembles,1))
#
    #next_cycle!.(new_ensembles)
#
    #tuning_init!.(new_tuners, new_ensembles, init_alg.nsteps_init) # TODO train flow here
##
    #@debug "Testing $(length(new_ensembles)) candidate MCMC ensemble(s)."
##
    #for i in 1:init_alg.nsteps_init # TODO Tempering # posterior = PosteriorDensity(posterior.likelihood,posterior.prior)
    #    transformed_mcmc_iterate!(
