...
@with_kw struct TransformedMCMCEnsemblePoolInit <: TransformedMCMCInitAlgorithm
    init_tries_per_walker::ClosedInterval{Int64} = ClosedInterval(8, 128)
...
function mcmc_init!(
    ...
    @debug "Generating $n $(init_tries > 1 ? "additional " : "")candidate MCMC walker(s) per ensemble."
    ...
    for ensemble in new_ensembles
        mask = (ensemble.n_accepted .> 0.05*init_alg.nsteps_init)
